...
> **Owner: Greg Rothwell. Last updated: 24 September 2026.**

**Links repaired 21 September 2026.** Entries arrive here verbatim from a
directory up, so every `decisions/…` path in them was silently broken on the way
— twelve of them, dating from the earlier archive passes, plus two more added the
same day. `MEMORY-PROTOCOL.md` says to check the links after a move; nobody had.
Paths only: not one word of an entry was changed.
...
**21 September 2026, a second pass.** The spine still carried leftover summaries
of 9 / 8 / 4 September after those days had been moved whole. Those summaries
are here so replacing them with pointers does not destroy the words:

- **2026-09-09.** iTunes Name that Tune (79 GB previews), Flags (76, hashed), Sleeves (52, mzstatic hotlink), picture retitled Fine Art, the vault topped up 206, all live as `index-6odlsKCm`; and On the box via TMDB untitled backdrops, built but not live. Moved whole on 10 September at 299/300.
- **2026-09-08.** `firstMs` shipped — the snap guess *shown* at the reveal, not banned (Greg chose exposure over restriction on cost) — and the A–D spam lottery that exposed; `games/{gameId}` built so a finished round survives its answers subcollection, and the A/B gap that forced it (test per-question things, never per-round); the melody round given **Hear it again**; and all of it live as `index-BDZpMBAG`, `check-rules` 65/65. Plus the two moved earlier — the melody round abandoned after four, the five-column picker. Moved whole on 10 September at 299/300.
- **2026-09-04 (three more).** "The rest of the file is stale" is not evidence about a line; melody and picture going live with the vault as the gate; and the `melody-round` branch itself. Moved whole on 10 September at 322/300.
- **2026-09-04.** The squad-write correction, the negatives paste (`check-rules` allow **PASS** after the paste, which is what made the two deny cases mean anything), and the lobby writing the squad plus a stake that can go below zero. Moved whole on 10 September at 313/300.
...
**Sixth batch, 24 September 2026.** The 21 September sleeves entry — the
oldest still whole in the spine — moved so the spine could take the Split or
Steal decision without passing 300. Verbatim; its one `decisions/…` link
repointed. Its closing "see the correction above" means the correction entry,
which is still in the spine.
...
or archive, and are now in [`recall/2026-09.md`](2026-09.md) verbatim.
...
<!-- Moved from TOTAL-RECALL.md on 11 September 2026, at 314 lines against a 300
     budget, to make room for the CUC4 entry. Verbatim; only where they live has
     changed. -->
...
<!-- Moved from TOTAL-RECALL.md on 11 September 2026, at 322 lines against a 300
     budget, to make room for the #52 deploy entry. Verbatim. -->

## 2026-09-10 — Live: volume, clip cuts, 177 songs (`index-C3jZR3XU`)

The tunes release, superseded the same day by the token-gate deploy and then by
CI. Archived whole: [`recall/2026-09.md`](2026-09.md).
...
Landed as #40. Archived whole: [`recall/2026-09.md`](2026-09.md).
